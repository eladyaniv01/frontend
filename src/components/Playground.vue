<template>
  <div class="row justify-left" style="width: 100%;">
    <q-dialog v-model="displayLocale">
      <q-card style="min-width: 300px;">
        <q-toolbar class="bg-blue-8 text-grey-1">
          <q-toolbar-title>
            Current local:
            <strong>{{ displayedLocale }}</strong>
          </q-toolbar-title>
        </q-toolbar>
        <q-card-section class="text-caption">Enter a new locale:</q-card-section>
        <q-card-section>
          <q-input v-model="locale" autofocus label="Locale"></q-input>
        </q-card-section>
      </q-card>
    </q-dialog>
    <q-btn dense label="Change locale" @click="displayLocale = true" class="col-12"></q-btn>
    <q-toggle class="col-12" v-model="fiveDayWorkWeek" label="5 day work week"></q-toggle>
    <q-toggle class="col-12" v-model="firstDayMonday" label="Week starts on Monday"></q-toggle>
    <q-toggle class="col-12" v-model="shortMonthLabel" label="Short month label"></q-toggle>
    <q-toggle class="col-12" v-model="shortWeekdayLabel" label="Short weekday label"></q-toggle>
    <q-toggle class="col-12" v-model="shortIntervalLabel" label="Short interval label"></q-toggle>
    <q-toggle class="col-12" v-model="format24h" label="24hr interval labels"></q-toggle>
    <q-toggle class="col-12" v-model="showWorkWeeks" label="Show work weeks"></q-toggle>
    <q-toggle class="col-12" v-model="hideHeader" label="Hide header"></q-toggle>
    <q-toggle class="col-12" v-model="showMonthLabel" label="Show month label"></q-toggle>
    <q-toggle class="col-12" v-model="showDayOfYearLabel" label="Show Day of Year Label"></q-toggle>
    ...
    <div class="col-12 q-px-md q-pb-sm">
      ...
      <q-slider v-model="dayHeight" :min="50" :max="200" label :label-value="dayHeight + 'px'"/>
    </div>
    <q-toggle class="col-12" v-model="enableThemes" label="Enable themes"></q-toggle>
    <q-select
      outlined
      dense
      emit-value
